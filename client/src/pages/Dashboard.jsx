...
import { LogOut, Activity, Users, Settings, UserPlus, AlertTriangle, Shield, Moon, Sun, AlertCircle, BarChart2, CheckCircle, X, Phone, MapPin, Radio, Siren } from 'lucide-react';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    newSocket.on('incident_claimed', (data) => {
      ...
      setTimeout(() => setClaimedAlertMsg(null), 8000);
    });

    // Staff SOS alert
    newSocket.on('staff_sos_alert', (data) => {
      setSosAlert(data);
      playEmergencyBuzzAlarm();
  ...
  const sendStaffSOS = () => {
    if (!socket) return;
    socket.emit('staff_sos', { name: user.name, role: user.role, domain: user.domain });
    setSosSent(true);
  ...
  return (
    <div className="dashboard-grid">
      ...
      {/* ── Staff SOS Alert Toast ── */}
      {sosAlert && (
        <div style={{
          position: 'fixed', top: '1rem', left: '50%', transform: 'translateX(-50%)', zIndex: 9999,
          background: '#ef4444', color: 'white', padding: '1rem 1.5rem',
          borderRadius: '10px', boxShadow: '0 4px 24px rgba(239,68,68,0.5)',
          display: 'flex', alignItems: 'center', gap: '0.75rem',
          minWidth: '340px', animation: 'slideIn 0.3s ease-out'
        }}>
          <span style={{ fontSize: '1.5rem' }}>🆘</span>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700 }}>STAFF SOS — {sosAlert.name} ({sosAlert.role})</div>
            <div style={{ fontSize: '0.8rem', opacity: 0.85 }}>Needs immediate assistance! Respond now.</div>
          </div>
          <button onClick={() => { setSosAlert(null); stopEmergencyBuzzAlarm(); }} style={{ background: 'rgba(0,0,0,0.2)', border: 'none', color: 'white', cursor: 'pointer', borderRadius: '4px', padding: '0.2rem 0.5rem' }}>
            <X size={16} />
          </button>
        </div>
      )}
      ...
      <aside className="sidebar">
        ...
        <nav style={{ flex: 1 }}>
          ...
          {(user.role === 'Administrator' || user.role === 'Hotel Manager' || user.role === 'Duty Manager' || user.role === 'Admin') && (
            <>
              ...
              <Link to="/dashboard/evacuation" className={`nav-link ${location.pathname === '/dashboard/evacuation' ? 'active' : ''}`}>
                <MapPin size={18} /> Evacuation Map
              </Link>
              ...
              <Link to="/dashboard/responder" className={`nav-link ${location.pathname === '/dashboard/responder' ? 'active' : ''}`}>
                <Radio size={18} /> Responder Link
              </Link>
            </>
          )}
        </nav>
        ...
        <div style={{ marginTop: 'auto' }}>
          ...
          {sosSent ? (
            <div style={{
              width: '100%', marginBottom: '0.75rem', padding: '0.75rem',
              background: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.4)',
              borderRadius: '8px', textAlign: 'center', color: '#ef4444', fontWeight: 700, fontSize: '0.85rem'
            }}>
              🆘 SOS Sent — Help Coming
            </div>
          ) : (
            <button
              onClick={sendStaffSOS}
              style={{
                width: '100%', marginBottom: '0.75rem', display: 'flex', alignItems: 'center',
                justifyContent: 'center', gap: '0.5rem', background: 'rgba(239,68,68,0.1)',
      ...
      <main className="main-content">
        <Routes>
          ...
          <Route path="/evacuation" element={<EvacuationDashboard socket={socket} user={user} />} />
          <Route path="/responder" element={<AdminDashboard section="responder" user={user} />} />
        </Routes>
      </main>
    </div>
  );
