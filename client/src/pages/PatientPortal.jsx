...
export default function PatientPortal() {
  ...
  const queryFloor = new URLSearchParams(location.search).get('floor');
  ...
  const [locationDetails, setLocationDetails] = useState(queryFloor || '');
  const isLocationLocked = !!queryFloor;
  ...
  useEffect(() => {
    ...
    // Try syncing immediately on load in case we just came online
    processQueue();
  ...
  const handleSubmit = async (e) => {
    ...
    if (geoStatus === 'checking') {
      alert(t('verifyingLocation'));
      return;
    }
    
    const canSubmit = geoStatus === 'ok' || geoStatus === 'bypass';
    if (!canSubmit) {
      setFormError(geoMessage || t('outsidePerimeter'));
      return;
    }
    ...
    const payloadData = {
      ...
      sessionId,
      floor: `Location: ${locationDetails}`.trim(),
    };

    console.log('[PatientPortal] Submitting SOS incident...', payloadData);
    ...
    if (!navigator.onLine) {
       ...
       setActiveIncident({
         id: 'offline-' + Date.now(),
         status: 'Pending',
         aiTriageInstructions: 'You are currently offline. Your SOS report has been saved locally and will transmit automatically once connection is restored. Please seek immediate shelter or find a staff member.',
       });
       setStep('success');
       setSubmitting(false);
       return;
    ...
    try {
      // POST JSON FIRST (Decoupling Media)
      const res = await fetch(`${API_BASE}/api/incident`, { 
        method: 'POST', 
        headers: { 'Content-Type': 'application/json' },
      ...
      if (res.ok) {
        ...
        // Asynchronously upload media if it exists
        if (file) {
           const fd = new FormData();
           fd.append('media', file);
           fetch(`${API_BASE}/api/incident/${createdData.id}/media`, { method: 'POST', body: fd })
             .catch(err => console.error('Media upload failed in background', err));
        }
      } else {
        const err = await res.json();
        setFormError(`DEBUG [Status ${res.status}]: ` + (err.error || t('failedSubmit')));
      }
    } catch {
        ...
        {step === 'form' && (geoStatus === 'ok' || geoStatus === 'bypass') && (
          <form className="portal-form" onSubmit={handleSubmit}>
            ...
            <div className="portal-select-wrap">
              <select className="portal-select" value={locationDetails} onChange={(e) => setLocationDetails(e.target.value)} required disabled={isLocationLocked}>
                <option value="">{t('selectFloor')}</option>
                {DOMAINS[session.domain || contextDomain]?.floors?.map(f => (
                  <option key={f} value={f}>{f}</option>
                ))}
                {!isLocationLocked && <option value="Other / Outside">{t('otherOutside')}</option>}
                {isLocationLocked && !DOMAINS[session.domain || contextDomain]?.floors?.includes(locationDetails) && (
