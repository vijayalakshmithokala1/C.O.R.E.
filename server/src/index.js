...
io.on('connection', (socket) => {
  ...
  socket.on('staff_sos', (data) => {
    const domain = data.domain || 'HOSPITAL';
    console.log(`[Socket] Staff SOS from ${data.name} (${data.role}) in ${domain}`);
    io.to(`staff_all_${domain}`).emit('staff_sos_alert', {
      name: data.name,
      role: data.role,
      domain,
...
app.use('/api/patient', sessionRoutes);
