...
// ── Generate First Responder Link ──────────────────────────────────────────
const jwt = require('jsonwebtoken');
router.post('/responder-link', async (req, res) => {
  try {
    const secret = process.env.RESPONDER_JWT_SECRET || process.env.JWT_SECRET;
    const token = jwt.sign(
      { domain: req.user.domain, type: 'responder' },
      secret,
      { expiresIn: '4h' }
    );
    const clientUrl = process.env.CLIENT_URL || 'http://localhost:5173';
    const url = `${clientUrl}/responder/${token}`;
    res.json({ url, token, expiresIn: '4 hours' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Failed to generate responder link' });
  }
});

// ── Revoke First Responder Links (Panic Button) ──────────────────────────
router.post('/responder-revoke', async (req, res) => {
  try {
    // Set a global memory variable. In production with multiple instances, this should be in Redis or DB.
    global.revokedResponderLinksBefore = Math.floor(Date.now() / 1000);
    
    await createChainedLog('Revoke Responder Links', `Admin revoked all active First Responder feeds`, req.user.id, req.user.domain);

    res.json({ success: true, message: 'All active responder links revoked' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Failed to revoke responder links' });
  }
});
